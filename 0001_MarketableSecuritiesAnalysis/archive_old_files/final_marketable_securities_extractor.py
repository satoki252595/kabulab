...
class FinalMarketableSecuritiesExtractor:
    ...
    def extract_company_info_from_xbrl(self, xbrl_content):
        """XBRLファイルから提出会社情報を抽出（改善版）"""
        soup = BeautifulSoup(xbrl_content, 'lxml-xml')
        
        # 提出会社のEDINETコード - 複数パターンを試す
        edinet_code = None
        edinet_patterns = [
            r'EDINETCode',
            r'CompanyCode',
        ...
        # ファイル名からEDINETコードを抽出する方法も試す
        if not edinet_code:
            # ファイル名パターンから抽出
            filename_match = re.search(r'_([E]\d+)-', xbrl_content)
            if filename_match:
                edinet_code = filename_match.group(1)
    ...
    def save_to_csv(self, filename='final_marketable_securities_analysis.csv'):
        """結果をCSVファイルに保存"""
        ...
        if not df.empty:
            ...
            print(f"Unique filing companies: {df['filing_company_code'].unique()}")
        else:
...
def main():
    ...
    # 最終版特定投資株式情報抽出器を初期化
    extractor = FinalMarketableSecuritiesExtractor(xbrl_folder_path)
    ...
    if not df.empty:
        print("\nSample data:")
        print(df[['filing_company_code', 'filing_company_name', 'held_security_name', 'book_value_million_yen']].head())
