...
class ImprovedMarketableSecuritiesExtractor:
    ...
    def extract_company_info_from_xbrl(self, xbrl_content):
        ...
        # 提出会社のEDINETコード
        edinet_code = None
        edinet_elements = soup.find_all('ix:nonNumeric', {'name': re.compile(r'EDINETCode')})
        if edinet_elements:
            edinet_code = edinet_elements[0].get_text(strip=True)
        
        # 提出会社名
        company_name = None
        company_elements = soup.find_all('ix:nonNumeric', {'name': re.compile(r'CompanyName')})
        if company_elements:
            company_name = company_elements[0].get_text(strip=True)
        
        # 証券コード
        stock_code = None
        stock_elements = soup.find_all('ix:nonNumeric', {'name': re.compile(r'SecurityCode')})
        if stock_elements:
            stock_code = stock_elements[0].get_text(strip=True)
        
        # 期間終了日
        filing_date = None
        date_elements = soup.find_all('ix:nonNumeric', {'name': re.compile(r'CurrentPeriodEndDate')})
        if date_elements:
            filing_date = date_elements[0].get_text(strip=True)
    ...
    def extract_marketable_securities_data(self, xbrl_content):
        """XBRLファイルから特定投資株式情報を抽出（複数パターン対応）"""
        soup = BeautifulSoup(xbrl_content, 'lxml-xml')
        securities_data = []
        
        # 複数の可能な特定投資株式のパターンを検索
        patterns = [
        ...
        # 重複を除去
        unique_contexts = set()
        unique_name_elements = []
        for elem in name_elements:
            context = elem.get('contextRef', '')
            if context not in unique_contexts:
                unique_contexts.add(context)
                unique_name_elements.append(elem)
        
        name_elements = unique_name_elements
        ...
        for name_elem in name_elements:
            if name_elem.get_text(strip=True):
                ...
                if any(keyword in contextRef for keyword in ['CurrentYear', 'Current', 'Instant']):
                    ...
                    row_identifier = row_match.group(1) if row_match else None
                    
                    # 対応する株式数を検索
                    current_shares = None
                    for shares_elem in shares_elements:
                        shares_context = shares_elem.get('contextRef', '')
                        if self._contexts_match(contextRef, shares_context):
                    ...
                    for book_elem in book_value_elements:
                        book_context = book_elem.get('contextRef', '')
                        if self._contexts_match(contextRef, book_context):
                    ...
                    for purpose_elem in purpose_elements:
                        purpose_context = purpose_elem.get('contextRef', '')
                        if self._contexts_match(contextRef, purpose_context):
                            purpose = purpose_elem.get_text(strip=True)
                            break
                    
                    # データが存在する場合のみ追加
                    if current_shares or current_book_value:
                        securities_data.append({
                            'held_security_name': name_elem.get_text(strip=True),
                            'held_stock_code': self._extract_stock_code(name_elem.get_text(strip=True)),
    ...
    def process_all_folders(self, max_folders=None):
        """すべてのXBRLフォルダを処理"""
    ...
    def save_to_csv(self, filename='improved_marketable_securities_analysis.csv'):
        """結果をCSVファイルに保存"""
...
def main():
    ...
    # テスト用に最初の10フォルダのみ処理
    extractor.process_all_folders(max_folders=10)
