...
class ImprovedMarketableSecuritiesExtractor:
    ...
    def load_edinet_code_map(self):
        ...
        try:
            # EDINETコードリストファイルを探す
            downloads_path = os.path.join(os.path.dirname(self.xbrl_folder_path), 'downloads')
            extracted_path = os.path.join(downloads_path, 'extracted')
            
            if os.path.exists(extracted_path):
                csv_files = [f for f in os.listdir(extracted_path) if f.endswith('.csv')]
                if csv_files:
                    csv_path = os.path.join(extracted_path, csv_files[0])
                    self.edinet_code_map = pd.read_csv(csv_path, encoding='shift_jis')
                    print(f"Loaded EDINET code map: {len(self.edinet_code_map)} companies")
        except Exception as e:
            print(f"Could not load EDINET code map: {e}")
            
    def get_stock_code_from_edinet(self, edinet_code):
        """EDINETコードから株式コードを取得"""
        if self.edinet_code_map is not None:
            try:
                ...
                if not match.empty:
                    stock_code = match.iloc[0]['証券コード']
                    if pd.notna(stock_code) and str(stock_code).strip() != '':
                        return str(stock_code).strip()
            except Exception as e:
                print(f"Error getting stock code for {edinet_code}: {e}")
        return None
    ...
    def get_company_name_from_edinet(self, edinet_code):
        """EDINETコードから会社名を取得"""
        if self.edinet_code_map is not None:
            try:
                match = self.edinet_code_map[self.edinet_code_map['ＥＤＩＮＥＴコード'] == edinet_code]
                if not match.empty:
                    return match.iloc[0]['提出者名']
            except Exception as e:
                print(f"Error getting company name for {edinet_code}: {e}")
        return None
    ...
    def extract_stock_code_from_name(self, security_name):
        ...
        if self.edinet_code_map is not None:
            try:
                ...
                print(f"Error searching stock code for {security_name}: {e}")
    ...
    def extract_company_fundamentals(self, security_name, edinet_code=None):
        ...
        try:
            ...
            print(f"Error getting fundamentals for {security_name}: {str(e)}")
                ...
                for root, dirs, files in os.walk(extract_path):
                    for file in files:
                        if file.endswith('_ixbrl.htm') and '0104010_honbun' in file:
                            ...
                            if company_info:
                                # 会社名を取得
                                company_name = self.get_company_name_from_edinet(company_info['company_code'])
                                
                                # 株式コードを取得
                                filing_stock_code = self.get_stock_code_from_edinet(company_info['company_code'])
                                ...
                                for security in securities_data:
                                    ...
                                    result = {
                                        ...
                                        'filing_stock_code': filing_stock_code,
    ...
    def save_to_csv(self, filename='improved_marketable_securities_data.csv'):
        """結果をCSVファイルに保存"""
...
def main():
    ...
    # 最初の10フォルダでテスト
    extractor.process_all_folders(limit=10)
    
    # 結果をCSVに保存
    extractor.save_to_csv('improved_marketable_securities_analysis.csv')
    ...
    if not df.empty:
        ...
        print(f"Securities with stock codes: {df['held_stock_code'].notna().sum()}")
